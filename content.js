...
chrome.runtime.onMessage.addListener((msg, _sender, sendResponse) => {
  ...
  if (msg?.type === 'EXEC_SMART_TASK') {
    executeSmartTask(msg.taskOutline, msg.options, msg.originalTaskPlan).then(r => sendResponse({ ok: true, data: r })).catch(e => sendResponse({ ok: false, error: String(e) }));
    return true;
  }
});
...
// 执行智能任务
async function executeSmartTask(taskOutline, options = {}, originalTaskPlan = null) {
  ...
  await executor.init();
  return await executor.executeSmartTask(taskOutline, options, originalTaskPlan);
}
